...
void TaskMonitor::PrintAll() const {
  ...
  std::cout << "=========================================================\n";
  std::cout << std::left << std::setw(10) << "Task ID" << std::setw(10)
            << "Execs" << std::setw(12) << "Total(ms)" << std::setw(10)
            << "Max(ms)" << std::setw(10) << "Min(ms)"
            << "Avg(ms)\n";
  std::cout << "---------------------------------------------------------\n";
  ...
  if (m_stats.empty()) {
    ...
    for (const auto& [id, stats] : m_stats) {
      ...
      uint64_t display_min =
          (stats.min_time == UINT64_MAX) ? 0 : stats.min_time;

      std::cout << std::left << std::setw(10) << id << std::setw(10)
                << stats.exec_count << std::setw(12) << stats.total_time
                << std::setw(10) << stats.max_time << std::setw(10)
                << display_min << std::fixed << std::setprecision(2)
                << stats.AvgTime() << "\n";
    }
  }
  std::cout << "=========================================================\n\n";
