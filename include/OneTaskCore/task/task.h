...
class Task {
 public:
  Task(uint32_t id, uint8_t priority = 0)
      : m_id(id), m_priority(priority), m_state(TaskState::CREATED) {}
  ...
  uint64_t GetNextRunTime() const { return m_next_run_time; }
  void SetNextRunTime(uint64_t t) { m_next_run_time = t; }

  void SetPeriod(uint64_t period) { m_period = period; }
  uint64_t GetPeriod() const { return m_period; }

  bool IsPeriodic() const { return m_period > 0; }
